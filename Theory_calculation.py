...
####和M16的数据比较
    ...
    plt.plot(ms_use,np.log10(red_Mh_ms1)+delta_value,'r-',label=r'$QE-M_\ast-red$')
    plt.plot(ms_use,np.log10(blue_Mh_ms1)+delta_value,'b-',label=r'$QE-M_\ast-blue$')
    plt.plot(ms_use,np.log10(red_Mh_ms2)+delta_value,'r-.',label=r'$QE-M_h-red$')
    plt.plot(ms_use,np.log10(blue_Mh_ms2)+delta_value,'b-.',label=r'$QE-M_h-blue$')
    plt.xlabel(r'$lgM_\ast [M_\odot ]$')
    plt.ylabel(r'$lgM_h [M_\odot h^{-1}]$')
    plt.legend(loc=2)
    plt.savefig('Theory_compare_data',dpi=600)
    plt.show()
    return
#########################
def control_file(R):
